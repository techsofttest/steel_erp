...
';
    ...
    if ($pageCount == 1) {
        ...
        $mpdf = new \Mpdf\Mpdf([    
            'margin_top' => 68, 
            'margin_bottom' => 120,  // Larger margin to accommodate last_page_footer
            'margin_header' => 10,
            //'margin_footer' => 40,
            'margin_left' => 5,
            'margin_right' => 5,
        ...
        $mpdf->SetAutoPageBreak(true, 120);
        ...
        //$mpdf->WriteHTML('<div style="position: fixed; bottom: 25mm; left: 5mm; right: 5mm; background: white;">' . $last_page_footer . '</div>');
    ...
    // First pass: Create PDF with smaller footer to check page count
    $mpdf_test = new \Mpdf\Mpdf([    
        'margin_top' => 68,
        'margin_header' => 10,
        'margin_left' => 5,
        'margin_right' => 5,
    ...
    $mpdf_test->SetAutoPageBreak(true, 40);
    $mpdf_test->SetHTMLHeader($header_html);
    $mpdf_test->WriteHTML($main_table);
    
    // Check the page count
    $page_count = $mpdf_test->page;
    unset($mpdf_test);

    // Second pass: Create the actual PDF with appropriate margins
    $bottom_margin = ($page_count == 1) ? 90 : 40;
    ...
    $mpdf = new \Mpdf\Mpdf([    
        ...
        'defaultfooterline' => 0,
        'setAutoTopMargin' => 'stretch',
        'setAutoBottomMargin' => $bottom_margin,
    ]);

    $mpdf->SetAutoPageBreak(true, $bottom_margin);
    $mpdf->SetTitle($title);

    $last_page_footer = '
    <table style="border-top:1px solid; border-collapse: collapse; width: 100%; font-size: 12px;margin-left:20px;margin-right:20px;">
        <tr>
    ...
    <table style="border-top:1px solid; border-collapse: collapse; width: 100%; margin-top:0px;margin-left:20px;margin-right:20px;">
        <tr>
            <td>Antony Raphel - Production In-charge</td>
            <td style="text-align:right;">Justin Jose - Operations Manager</td>
        </tr>
